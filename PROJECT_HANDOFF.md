# 종로 도보 동선 추천 (가칭: Jongno Walk-Course Recommender) — 프로젝트 핸드오프 문서 **v6**
...
| L3 | 시퀀스 모델 (LSTM / 소형 Transformer) | ⬜ 보너스(선택) |
...
**미구현 / 다음 후보:**
1. **NN L3 (LSTM / 소형 Transformer)** — 방문 "순서" 모델링. **보너스/선택**(L2+고전ML 비교로 NN 파트 이미 발표 충분). Colab 재세팅(노트북 모두 실행 + CSV 재업로드) 필요.
2. **데이터 확장 (B)** — 서점·쇼핑·문화시설(박물관·도서관) 추가 → 노드 재구성 → OD 재계산 → JSON 재생성. ⚠️ **반나절 작업, 위험**(파이프라인 재실행 + 교보문고 재편입 충돌 + 노드캡 재배분). 교보문고 서점 재분류도 이때. **후순위.**
3. **백엔드 FastAPI (D22)** — 도보거리 스냅·라우팅.
4. **마무리:** OD 경량화(od.json 5.3MB) · GitHub Pages 배포(vite base 설정 + VWorld 도메인 추가등록) · README 영어화 · 라이선스(MIT) · 위성지도 토글 · NN 랭커 앱 연결(C 레이어).
