# 종로 도보 동선 추천 (가칭: Jongno Walk-Course Recommender) — 프로젝트 핸드오프 문서 **v9**
...
>
> **v9 변경 요지:** 포트폴리오 마감 일괄 — MIT 라이선스 · '학교 과제'→'실습 과제' 문구 정리 · 일반↔위성 지도 토글 · 포트폴리오급 README 전면 재작성(영문 README.md + 한글 README.ko.md, 배지·NN 비교표·앱 스크린샷 3장) · Hugging Face NN 데모 배포(L2 랭커 Gradio, huggingface.co/spaces/urban4isw/jongno-nn-ranker) · GitHub About/토픽 · GitHub Actions 자동배포(.github/workflows/deploy.yml, main push 시 자동 빌드·배포, VWorld 키는 Secrets). OD 경량화는 이미 gzip 1.7MB라 불필요 확인(스킵). 프로젝트 포트폴리오 완성 상태.
...
**배포 방식:** ~~gh-pages 수동(`npm run deploy`)~~ → **GitHub Actions 자동**(`.github/workflows/deploy.yml`, main push 시 빌드·배포). **Pages 소스=GitHub Actions**, **VWorld 키는 레포 Secrets `VITE_VWORLD_KEY`** 로 주입. (수동 `npm run deploy`는 폴백으로 남겨둠.)
...
- **포트폴리오 마감(v9):** 일반↔위성 지도 토글 · 포트폴리오급 README 전면 재작성(영문 README.md + 한글 README.ko.md, 배지·NN 비교표·앱 스크린샷 3장 docs/) · GitHub About/토픽 정리 · **GitHub Actions 자동배포**(.github/workflows/deploy.yml) · **MIT 라이선스** · '학교 과제'→'실습 과제' 문구 정리. OD 경량화는 이미 gzip 1.7MB라 스킵.
- 전부 GitHub push·동기화. (최신 커밋: `d0388e7`)

> ✅ **프로젝트 포트폴리오 완성 상태.** 아래는 모두 **선택적 확장**(필수 아님).

**선택적 확장 (필요 시):**
1. **NN 랭커 앱 연결 (C 레이어)** — Foursquare 검증 모델/신호를 종로 앱 추천에 실제 연결.
2. **백엔드 FastAPI (D22)** — 도보거리 스냅·라우팅.
3. **네이버 인기신호 보조** — 지역 API 인기순 top5를 가중치로만(5건 캡).
4. **사용자 신호·인기도 조건화** — v1.2(슬라이더 가중) / v1.3(성·연령 조건화).
...
- **v9:** 포트폴리오 마감 — MIT 라이선스 · '학교'→'실습 과제' 문구 정리 · 일반↔위성 토글 · README 전면 재작성(영문+한글, 배지·NN표·스크린샷3) · HF NN 데모 배포(L2 Gradio) · GitHub About/토픽 · GitHub Actions 자동배포(Secrets). OD 경량화는 gzip 1.7MB라 스킵. **포트폴리오 완성**, 이후는 선택적 확장.
- 이후 변경은 여기에 한 줄씩 추가.
